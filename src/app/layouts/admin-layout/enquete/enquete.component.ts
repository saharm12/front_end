...
export class EnqueteComponent implements OnInit {
  question: any = [
    { id: 1, text: " Avez-vous apprécié l évenement (date,lieu,animateur..)" },
    {
      id: 2,
      text: " Avez-vous apprécié l organisation de l évenement par iceberg",
    },
    {
      id: 3,
      text: " Pensez-vous participer a une autre de nos événement a l avenir",
    },
    { id: 4, text: " Recommanderiez-vous nos évenement a un ami" },
  ];
  p: number = 1;
  ...
  changeQuestion(idQuestion) {
    ...
    this.satisfait = this.enquetes.filter(
      (c) => c.Rep == "A" && c.numQuestion == idQuestion
    ).length;
    this.moyennementSatisfait = this.enquetes.filter(
      (c) => c.Rep == "B" && c.numQuestion == idQuestion
    ).length;
    this.faiblementSatisfait = this.enquetes.filter(
      (c) => c.Rep == "C" && c.numQuestion == idQuestion
    ).length;
    console.log(
      this.satisfait,
      ":",
